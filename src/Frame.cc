...
//run this
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const cv::Mat &imColor, const cv::Mat &mask)
    :mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mRawImage(imColor.clone()), mQuadricImage(imColor.clone())
{
    // Frame ID
    mnId=nNextId++;
...
void Frame::ReadDetections()
{
    char filename[256];
    std::string bbox_file_path;

    bbox_file_path = paras.str_seq_path + "/yolo_txt/";
    sprintf(filename, "%s%06lu.txt", bbox_file_path.c_str(), mnId);
    ...
    for(int i=0; i<detection_mat_trans.cols(); i++)
    {
        ...
        // obj->PrintInfo();
        mvpFrameDetections.push_back(obj);
    }

    //printf("frame_id: %d\n", mnId);
}

void Frame::ReadSegDetections()
{
    char filename[256];
    std::string bbox_file_path;

    bbox_file_path = paras.str_seq_path + "/segmentation_txt/";
    sprintf(filename, "%s%06lu.txt", bbox_file_path.c_str(), mnId);
    Eigen::MatrixXd detection_mat, detection_mat_trans;
...
std::vector<MapPoint*> Frame::GetFrameMapPoints()
{
