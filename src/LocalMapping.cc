...
{
    std::vector<MapObject*> MapObjects = mpCurrentKeyFrame->GetMapObjects();
    for(int i=0;i<MapObjects.size();i++){
        auto obj = MapObjects[i];
        cv::Rect curr_rect = mpCurrentKeyFrame->mvpLocalObjects_2D[i]->mBoxRect;
        ...
        std::vector<MapPoint*> vMPs = obj->GetAssociatedMP();
        auto vIndices_in_box = mpCurrentKeyFrame->GetFeaturesInBox(bb_det[0], bb_det[2], bb_det[1], bb_det[3]);
        SearchInBox(mpCurrentKeyFrame, vMPs, vIndices_in_box);
    }
}

/*
    vpMapPoints: 3d object map points
    vIndices_in_box: keypoints in box 
*/
void LocalMapping::SearchInBox(KeyFrame *pKF, const vector<MapPoint*> &vpMapPoints, const vector<size_t> vIndices_in_box)
{
...
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        // skip over the KF which observed a new object
        if(pKF->AppearNewObject)
            continue;
...
void LocalMapping::UpdateObject()
{
    ...
    const std::vector<MapObject*> &vObjs = mpMap->GetAllMapObjects();
    ...
    for(size_t i = 0; i < vObjs.size(); ++i)
    {
        ...
        if((Obj->mspAssociatedMP.size() < 10) || (Obj->isBad()))
        {
            continue;
        }
...
void LocalMapping::MergePotentialAssObjs()
{
    ...
    for(size_t i = 0; i < vObjs.size(); ++i)
    {
        ...
        if (Obj->Observations() >= 5)     // todo: when to do ?
        {
            if(Obj->mReObj.size() > 0)  //todo 二次检验依然是在潜在关联对象中找的。
            {
...
void LocalMapping::WhetherOverlapObject()
{
    ...
    for(size_t i = 0; i < vObjs.size(); ++i)
    {
        ...
        if((Obj->mspAssociatedMP.size() < 10) || (Obj->isBad()) || (Obj->Observations() < 5))
        {
            continue;
        }

        for(size_t j = 0; j < vObjs.size(); ++j)
        {
            if(i == j)
                continue;
            ...
            if((Obj2->mspAssociatedMP.size() < 10) || (Obj2->isBad()) || (Obj2->Observations() < 5))
            {
                continue;
            }
            
            float dis_x = abs(Obj->mGeometricPosition(0) - Obj2->mGeometricPosition(0));
            float dis_y = abs(Obj->mGeometricPosition(1) - Obj2->mGeometricPosition(1));
            float dis_z = abs(Obj->mGeometricPosition(2) - Obj2->mGeometricPosition(2));

            float sum_lenth_half = Obj->mpQuadric->scale(0)/2 + Obj2->mpQuadric->scale(0)/2;
            float sum_width_half = Obj->mpQuadric->scale(1)/2 + Obj2->mpQuadric->scale(1)/2;
            float sum_height_half = Obj->mpQuadric->scale(2)/2 + Obj2->mpQuadric->scale(2)/2;

            float fVolume = (Obj->mpQuadric->scale(0) * Obj->mpQuadric->scale(1)) * Obj->mpQuadric->scale(2);
            float fVolume2 = (Obj2->mpQuadric->scale(0) * Obj2->mpQuadric->scale(1)) * Obj2->mpQuadric->scale(2);
